    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="lg:grid lg:grid-cols-12 lg:gap-x-5">
            ...
            <aside class="py-6 px-2 sm:px-6 lg:py-0 lg:px-0 lg:col-span-3">
                <nav class="space-y-1">
                    <a href="{{ route('profile.index') }}" class="text-gray-900 group rounded-md px-3 py-2 flex items-center text-sm font-medium hover:bg-gray-50 hover:text-gray-900">
                        <!-- Icon -->
                    ...
                    <a href="{{ route('orders.index') }}" class="text-gray-900 group rounded-md px-3 py-2 flex items-center text-sm font-medium hover:bg-gray-50 hover:text-gray-900">
                        <!-- Icon -->
                        <svg class="text-gray-400 group-hover:text-gray-500 flex-shrink-0 -ml-1 mr-3 h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                        <span class="truncate">Order History</span>
                    </a>

                    <a href="{{ route('wishlist.index') }}" class="bg-orange-50 text-orange-700 hover:bg-white group rounded-md px-3 py-2 flex items-center text-sm font-medium border-l-4 border-orange-600 shadow-sm">
            ...
            <div class="sm:px-6 lg:px-0 lg:col-span-9">
                    ...
                    <div class="p-6">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-20 w-20 bg-gray-200 rounded-md overflow-hidden flex items-center justify-center">
                                <span class="text-xs text-text-gray-500 px-1 text-center">{{ $item->product->name }}</span>
                            </div>
                            <div class="ml-6 flex-1">
                                <div class="flex items-center justify-between">
                                    <h3 class="text-lg font-medium text-gray-900">
                                        <a href="{{ route('products.show', $item->product->id) }}" class="hover:text-blue-600 transition">
                                            {{ $item->product->name }}
                                        </a>
                                    </h3>
                                    <p class="text-lg font-extrabold text-orange-600">Rp {{ number_format($item->product->price, 0, ',', '.') }}</p>
                                </div>
                                <p class="mt-1 text-sm text-gray-500 truncate max-w-md">{{ $item->product->description }}</p>
                                <div class="mt-4 flex items-center justify-between">
                                    <div class="flex space-x-4">
                                        <a href="{{ route('cart.add', $item->product->id) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-bold rounded-xl shadow-sm text-white bg-orange-600 hover:bg-orange-700 transition">
                                            Masukkan ke Keranjang
                                        </a>
                                        <a href="{{ route('wishlist.remove', $item->product->id) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                            Remove
                                        </a>
                                    </div>
                                    <div class="text-sm">
                ...
                <div class="bg-white shadow rounded-lg p-12 text-center">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('products.index') }}" class="inline-flex items-center px-6 py-3 border border-transparent shadow-sm text-sm font-bold rounded-xl text-white bg-orange-600 hover:bg-orange-700 transition">
                            Cari KhilafStore
                        </a>
                    </div>
                </div>
