...
<div class="space-y-8 pb-10">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-extrabold text-[#1A1C1E]">Sales Analytics</h2>
            <p class="text-sm text-gray-400 mt-1 font-medium italic">Comprehensive performance metrics for your business.</p>
        </div>
        <button onclick="window.print()" class="flex items-center gap-2 px-4 py-2 bg-white border border-gray-100 rounded-xl text-gray-600 font-bold hover:bg-gray-50 transition shadow-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <h3 class="text-2xl font-extrabold text-[#1A1C1E]">Rp {{ number_format($lifetimeRevenue, 0, ',', '.') }}</h3>
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <h3 class="text-2xl font-extrabold text-[#1A1C1E]">Rp {{ number_format($averageOrderValue, 0, ',', '.') }}</h3>
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            ...
            @endphp
            <h3 class="text-2xl font-extrabold text-[#1A1C1E]">{{ $bestMonth }}</h3>
        ...
        <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            ...
            @endphp
            <h3 class="text-2xl font-extrabold text-[#1A1C1E]">{{ $topCategory->name ?? 'None' }}</h3>
        ...
        <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <div class="mt-6 space-y-3">
                ...
                <div class="flex items-center justify-between text-xs">
                    <span class="text-gray-500 font-bold uppercase tracking-widest">{{ $stat->name }}</span>
                    <span class="text-[#1A1C1E] font-extrabold">Rp {{ number_format($stat->revenue, 0, ',', '.') }}</span>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-[#F8F9FA]">
                        <th class="px-8 py-4 text-[10px] font-extrabold uppercase tracking-widest text-gray-400">products Info</th>
                        <th class="px-8 py-4 text-[10px] font-extrabold uppercase tracking-widest text-gray-400 text-center">Units Sold</th>
                        <th class="px-8 py-4 text-[10px] font-extrabold uppercase tracking-widest text-gray-400 text-right">Revenue</th>
                    ...
                    <tr class="group hover:bg-[#F8F9FA] transition">
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-4">
                                @if($product->image)
                                <img src="{{ asset('storage/' . $product->image) }}" class="w-10 h-10 rounded-xl object-cover" alt="">
                                @else
                                <div class="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-gray-300">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                </div>
                                @endif
                                <div>
                                    <p class="text-sm font-extrabold text-[#1A1C1E]">{{ $product->name }}</p>
                                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{{ $product->category->name ?? 'Uncategorized' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-5 text-center font-extrabold text-[#1A1C1E]">
                            {{ $product->total_sold }}
                        </td>
                        <td class="px-8 py-5 text-right font-extrabold text-orange-600">
                            Rp {{ number_format($product->total_sold * $product->price, 0, ',', '.') }}
                        </td>
                        <td class="px-8 py-5 text-center">
                            @if($product->stock < 10)
                                <span class="px-3 py-1 bg-red-50 text-red-500 rounded-lg text-[10px] font-extrabold uppercase tracking-widest border border-red-100">Critical: {{ $product->stock }}</span>
                            @else
                                <span class="px-3 py-1 bg-orange-50 text-orange-500 rounded-lg text-[10px] font-extrabold uppercase tracking-widest border border-orange-100">Ok: {{ $product->stock }}</span>
                            @endif
                        </td>
                    </tr>
                    @endforeach
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const revenueTrendOptions = {
            series: [{
                name: 'Revenue',
                data: {!! json_encode($monthlyTrends) !!}
            }],
            chart: {
                height: 350,
                type: 'area',
        ...
        const categoryShareOptions = {
            series: {!! json_encode($categoryStats->pluck('revenue')) !!},
            chart: {
                type: 'donut',
                height: 350,
                fontFamily: 'Plus Jakarta Sans, sans-serif'
