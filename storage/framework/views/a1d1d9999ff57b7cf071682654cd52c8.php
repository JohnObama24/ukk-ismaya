        ...
        <div class="lg:grid lg:grid-cols-2 lg:gap-x-8 lg:items-start">
            ...
            <div class="flex flex-col-reverse">
                <div class="w-full aspect-square bg-gray-50 rounded-lg overflow-hidden border border-gray-200">
                     <?php if($product->image): ?>
                        <img src="<?php echo e(asset('storage/' . $product->image)); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-contain p-4">
                     <?php else: ?>
                         <div class="w-full h-full bg-gray-200 flex items-center justify-center text-2xl text-gray-500 font-bold">
                            <?php echo e($product->name); ?> Image
                        </div>
                     <?php endif; ?>
                </div>
                <div class="mt-4 flex items-center justify-center">
                    <a href="<?php echo e(route('wishlist.add', $product->id)); ?>" class="flex items-center gap-2 text-gray-500 hover:text-red-500 transition group">
            ...
            <div class="mt-10 px-4 sm:px-0 sm:mt-16 lg:mt-0">
                ...
                <div class="mt-3">
                    ...
                    <p class="text-3xl text-orange-600 font-extrabold">Rp <?php echo e(number_format($product->price, 0, ',', '.')); ?></p>
                </div>
                ...
                <div class="mt-6">
                    <div class="flex items-center mb-6">
                        ...
                        <span class="ml-4 text-sm text-gray-500">Tersedia <?php echo e($product->stock ?? 50); ?> buah</span>
                    </div>

                    <?php if(!auth()->check() || auth()->user()->role !== 'admin'): ?>
                    <div class="flex flex-col sm:flex-row gap-4 mt-8">
                        <form action="<?php echo e(route('cart.add', $product->id)); ?>" method="GET" id="addToCartForm" class="flex-1">
                            <?php echo csrf_field(); ?>
                        ...
                        <form action="<?php echo e(route('buy_now', $product->id)); ?>" method="POST" id="buyNowForm" class="flex-1">
                            <?php echo csrf_field(); ?>
                    ...
                    <div class="mt-10 p-4 bg-orange-50 border border-orange-200 rounded-xl">
                        ...
                        <div class="mt-4 flex flex-col sm:flex-row gap-3">
                            <a href="<?php echo e(route('admin.products.edit', $product->id)); ?>" class="inline-flex justify-center items-center px-4 py-2 bg-orange-600 border border-transparent rounded-lg font-bold text-white hover:bg-orange-700 transition shadow-sm text-sm">
                                Ubah Produk Ini
                            </a>
                            <a href="<?php echo e(route('admin.dashboard')); ?>" class="inline-flex justify-center items-center px-4 py-2 border border-gray-300 rounded-lg font-bold text-gray-700 bg-white hover:bg-gray-50 transition shadow-sm text-sm">
                     ...
                     <div class="py-6">
                        <div class="md:flex md:items-center md:justify-between mb-6">
                            <h3 class="text-xl font-bold text-gray-900">
                                Penilaian Produk (<?php echo e($ratings->count()); ?>)
                            </h3>
                            <div class="flex items-center gap-2 mt-2 md:mt-0">
                                ...
                                <span class="text-sm font-medium text-gray-500"><?php echo e(number_format($product->average_rating, 1)); ?> dari 5</span>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 p-6 rounded-xl mb-8 border border-gray-100">
                             <form action="<?php echo e(route('products.ratings.store', $product->id)); ?>" method="POST" enctype="multipart/form-data">
                                <?php echo csrf_field(); ?>
                                <h4 class="text-sm font-bold text-gray-900 mb-4 uppercase tracking-wide">Beri Penilaian</h4>
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Rating</label>
                                    <div class="flex gap-4">
                                        <?php $__currentLoopData = [5,4,3,2,1]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="rating" value="<?php echo e($r); ?>" class="mr-1 accent-orange-600" required>
                                            <span class="text-sm text-gray-600"><?php echo e($r); ?> Bintang</span>
                                        </label>
                            ...
                            <div class="flex gap-4 border-b border-gray-100 pb-6 last:border-0 last:pb-0">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center justify-between mb-1">
                                        <h4 class="text-sm font-bold text-gray-900"><?php echo e($rating->user->name); ?></h4>
                                        <span class="text-xs text-gray-400"><?php echo e($rating->created_at->diffForHumans()); ?></span>
                                    </div>
                                    <div class="flex text-yellow-400 mb-2">
                                        <?php for($i = 0; $i < 5; $i++): ?>
                                            <svg class="w-3 h-3 <?php echo e($i < $rating->rating ? 'fill-current' : 'text-gray-200'); ?>" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                        <?php endfor; ?>
                                    </div>
                                    <p class="text-sm text-gray-600 leading-relaxed mb-2"><?php echo e($rating->comment); ?></p>
                                    <?php if($rating->image): ?>
                                        <div class="mt-2">
                                            <a href="<?php echo e(asset('storage/' . $rating->image)); ?>" target="_blank" class="block w-24 h-24 rounded-lg overflow-hidden border border-gray-200 hover:border-orange-500 transition">
                                                <img src="<?php echo e(asset('storage/' . $rating->image)); ?>" alt="Rating Image" class="w-full h-full object-cover">
                                            </a>
                                        </div>
                                    <?php endif; ?>
                                </div>
        ...
        <section aria-labelledby="related-heading" class="mt-16 sm:mt-24">
            ...
            <div class="mt-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-2">
                ...
                <div class="group relative flex flex-col bg-white hover:shadow-lg transition shadow-sm border border-transparent hover:border-orange-500 duration-200">
                    <!-- Product Image -->
                    <a href="<?php echo e(route('products.show', $related->slug)); ?>" class="relative aspect-square bg-white block">
                        <div class="absolute inset-0 flex items-center justify-center">
                             <?php if($related->image): ?>
                                <img src="<?php echo e(asset('storage/' . $related->image)); ?>" alt="<?php echo e($related->name); ?>" class="w-full h-full object-contain p-4">
                             <?php else: ?>
                    ...
                    <div class="p-2 flex flex-col flex-1">
                        ...
                        <div class="mt-auto">
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <span class="text-sm font-semibold text-orange-600">Rp <?php echo e(number_format($related->price, 0, ',', '.')); ?></span>
                                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Project\Project-UKK\resources\views\products\show.blade.php ENDPATH**/ ?>